...
class WSAL_AuditLogListView extends WP_List_Table {
		...
		if ( Settings_Helper::is_archiving_enabled() ) {
			if (
				...
				<div class="wsal-ssa wsal-db">
					<select class="wsal-db" onchange="WsalDBChange(value, '<?php echo \esc_attr( \wp_create_nonce( 'wsal-switch-db' ) ); ?>');">
						<option value="live" <?php selected( 'live', $selected ); ?>>
							<?php esc_html_e( 'Live Database', 'wp-security-audit-log' ); ?>
						</option>
						<option value="archive" <?php selected( 'archive', $selected ); ?>>
	...
	public function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			case 'type':
				$desc = Alert_Manager::get_alert_property( $item['alert_id'], 'desc' );
				if ( false === $desc ) {
			...
			case 'code':
				$const = Constants::get_severity_by_code( Alert_Manager::get_alert_property( $item['alert_id'], 'severity' ) );
			...
			case 'user':
				$username = WSAL_Utilities_UsersUtils::get_username( $item['meta_values'] );
				...
				if ( $user instanceof WP_User ) {
					...
					if ( class_exists( 'WSAL_SearchExtension' ) ) {
						$tooltip .= esc_attr__( 'Show me all activity by this User', 'wp-security-audit-log' ) . '</br>';

						$uhtml = '<a class="search-user" data-tooltip="' . esc_attr( $tooltip ) . '" data-user="' . $user->user_login . '" href="' . $user_edit_link . '" target="_blank">' . esc_html( $display_name ) . '</a>';
					}
					/* @premium:end */
			...
			case 'mesg':
				$event_meta = $item['meta_values'];
				$result     = '<div id="Event' . $item['id'] . '">' . WSAL_Models_Occurrence::get_alert_message( $item ) . '</div>';
