...
class WSAL_Settings {
	 ...
	 *
	 * @since  4.1.0
	 * @deprecated 4.4.3 - Use self::get_working_dir_path_static()
	 */
	public function get_working_dir_path( $path = '', $skip_creation = false, $ignore_site = false ) {
		_deprecated_function( __FUNCTION__, '4.4.3', '\WSAL_Settings::get_working_dir_path_static()' );

		return self::get_working_dir_path_static( $path, $skip_creation, $ignore_site );
	}
	 ...
	 * @since 4.4.3
	 *
	 * @deprecated 4.5 - Use \WSAL\Helpers\Settings_Helper::get_working_dir_path_static()
	 */
	public static function get_working_dir_path_static( $path = '', $skip_creation = false, $ignore_site = false ) {
		_deprecated_function( __FUNCTION__, '4.5', '\WSAL\Helpers\Settings_Helper::get_working_dir_path_static()' );

		return \WSAL\Helpers\Settings_Helper::get_working_dir_path_static( $path, $skip_creation, $ignore_site );
	}
	/* @premium:end */
	 ...
	 */
	public function switch_to_archive_db() {
		if ( ! Settings_Helper::is_archiving_enabled() || is_null( $this->plugin->external_db_util ) ) {
			return;
		}

		WSAL_Connector_ConnectorFactory::enable_archive_mode();
	}

	/**
	 * Close Archive DB.
	 */
	public function close_archive_db() {
		if ( ! Settings_Helper::is_archiving_enabled() || is_null( $this->plugin->external_db_util ) ) {
			return;
		}

		$connection_name = WSAL\Helpers\Settings_Helper::get_option_value( 'archive-connection' );
		$connection      = $this->plugin->external_db_util->get_connection( $connection_name );
		if ( is_array( $connection ) ) {
			$this->plugin->get_connector( $connection )->close_connection();
			WSAL_Connector_ConnectorFactory::disable_archive_mode();
		}
	}
	 ...
	 *
	 * @since 3.1.2
	 * @deprecated 4.4.3 - Use \WSAL\Helpers\File_Helper::create_htaccess_file()
	 */
	final public function create_htaccess_file( $dir_path ) {
		_deprecated_function( __FUNCTION__, '4.4.3', '\WSAL\Helpers\File_Helper::create_htaccess_file()' );

		return \WSAL\Helpers\File_Helper::create_htaccess_file( $dir_path );
	}
	 ...
	 * @since 4.3.2
	 */
	public function is_database_logging_enabled() {
		$is_disabled = \WSAL\Helpers\Settings_Helper::get_boolean_option_value( 'db_logging_disabled', false );
		// If the logging is enabled, we don't need to check mirrors.
		if ( ! $is_disabled || is_null( $this->plugin->external_db_util ) ) {
			return true;
		}
	 ...
	 * @since 4.3.2
	 */
	public function set_database_logging_disabled( $is_disabled ) {
		if ( $is_disabled ) {
			\WSAL\Helpers\Settings_Helper::set_boolean_option_value( 'db_logging_disabled', $is_disabled );
			Alert_Manager::trigger_event( 6327, array( 'EventType' => 'disabled' ) );
		} else {
			\WSAL\Helpers\Settings_Helper::delete_option_value( 'db_logging_disabled' );
	...
	/* @premium:start */
	/**
	 * Checking if an empty email for periodic reports is enabled.
	 *
	 * @return bool
	 *
	 * @since 4.4.0
	 */
	public function is_empty_email_for_periodic_reports_enabled() {
		return \WSAL\Helpers\Settings_Helper::get_boolean_option_value( 'periodic-reports-empty-emails-enabled', false );
	}

	/**
	 * Sets the plugin setting that allows an empty email for periodic reports.
	 *
	 * @param mixed $enabled New value of the setting.
	 *
	 * @since 4.4.0
	 */
	public function set_empty_email_for_periodic_reports_enabled( $enabled ) {
		\WSAL\Helpers\Settings_Helper::set_boolean_option_value( 'periodic-reports-empty-emails-enabled', $enabled );
	}

	/**
	 * Checking if the pruning of old reports is enabled.
	 *
	 * @return bool
	 *
	 * @since 4.4.0
	 */
	public function is_report_pruning_enabled() {
		return \WSAL\Helpers\Settings_Helper::get_boolean_option_value( 'reports-pruning-enabled', true );
	}

	/**
	 * Sets the plugin setting that allows the pruning of old reports.
	 *
	 * @param mixed $enabled New value of the setting.
	 *
	 * @since 4.4.0
	 */
	public function set_report_pruning_enabled( $enabled ) {
		$boolean_value = Settings_Helper::string_to_bool( $enabled );
		\WSAL\Helpers\Settings_Helper::set_boolean_option_value( 'reports-pruning-enabled', $boolean_value );

		if ( ! $boolean_value ) {
			// Cancel the report pruning cron job.
			wp_clear_scheduled_hook( WSAL_Rep_Common::SCHEDULED_HOOK_REPORTS_PRUNING );
		}
	}

	/**
	 * Retrieves maximum age of report (number of days) for the automatic report removal.
	 *
	 * @return int Number of days.
	 *
	 * @since 4.4.0
	 */
	public function get_report_pruning_threshold() {
		return (int) WSAL\Helpers\Settings_Helper::get_option_value( 'reports-pruning-threshold', 30 );
	}

	/**
	 * Updates the maximum age of report (number of days) for the automatic report removal.
	 *
	 * @param int $days Number of days.
	 *
	 * @since 4.4.0
	 */
	public function set_report_pruning_threshold( $days ) {
		\WSAL\Helpers\Settings_Helper::set_option_value( 'reports-pruning-threshold', (int) $days );
	}

	/**
	 * Retrieves the time (hour fo the day) when the periodic reports should be sent.
	 *
	 * @return int Hour of day.
	 *
	 * @since 4.4.0
	 */
	public function get_periodic_reports_hour_of_day() {
		return (int) WSAL\Helpers\Settings_Helper::get_option_value( 'periodic-reports-hour', 2 );
	}

	/**
	 * Updates the time (hour fo the day) when the periodic reports should be sent.
	 *
	 * @param int $hour Hour of day.
	 *
	 * @since 4.4.0
	 */
	public function set_periodic_reports_hour_of_day( $hour ) {
		\WSAL\Helpers\Settings_Helper::set_option_value( 'periodic-reports-hour', (int) $hour );
	}

	/**
	 * Retrieves the URL of custom logo for the reports.
	 *
	 * @return string Image URL.
	 *
	 * @since 4.4.0
	 */
	public function get_custom_reports_logo() {
		return WSAL\Helpers\Settings_Helper::get_option_value( 'reports-custom-logo', '' );
	}

	/**
	 * Updates the URL of custom logo for the reports.
	 *
	 * @param string $url Image URL.
	 *
	 * @since 4.4.0
	 */
	public function set_custom_reports_logo( $url ) {
		if ( 0 === strlen( $url ) ) {
			\WSAL\Helpers\Settings_Helper::delete_option_value( 'reports-custom-logo' );
		} else {
			\WSAL\Helpers\Settings_Helper::set_option_value( 'reports-custom-logo', $url );
		}
	}

	/**
	 * Retrieves the target URL of custom logo for the reports.
	 *
	 * @return string Target site URL.
	 *
	 * @since 4.4.0
	 */
	public function get_custom_reports_logo_link() {
		return WSAL\Helpers\Settings_Helper::get_option_value( 'reports-custom-logo-link', '' );
	}

	/**
	 * Updates the target URL of custom logo for the reports.
	 *
	 * @param string $url Target site URL.
	 *
	 * @since 4.4.0
	 */
	public function set_custom_reports_logo_link( $url ) {
		if ( 0 === strlen( $url ) ) {
			\WSAL\Helpers\Settings_Helper::delete_option_value( 'reports-custom-logo-link' );
		} else {
			\WSAL\Helpers\Settings_Helper::set_option_value( 'reports-custom-logo-link', $url );
		}
	}

	/**
	 * Retrieves the business name for the reports.
	 *
	...
	public function get_business_name() {
		return WSAL\Helpers\Settings_Helper::get_option_value( 'reports-business-name', '' );
	}

	/**
	 * Updates the business name for the reports.
	 *
	...
	public function set_business_name( $value ) {
		if ( 0 === strlen( $value ) ) {
			\WSAL\Helpers\Settings_Helper::delete_option_value( 'reports-business-name' );
		} else {
			\WSAL\Helpers\Settings_Helper::set_option_value( 'reports-business-name', $value );
		}
	}

	/**
	 * Retrieves the contact name and surname for the reports.
	 *
	 * @return string Contact name and surname.
	 *
	 * @since 4.4.2.1
	 */
	public function get_contact_name() {
		return WSAL\Helpers\Settings_Helper::get_option_value( 'reports-contact-name', '' );
	}

	/**
	 * Updates the contact name and surname for the reports.
	 *
	...
	public function set_contact_name( $value ) {
		if ( 0 === strlen( $value ) ) {
			\WSAL\Helpers\Settings_Helper::delete_option_value( 'reports-contact-name' );
		} else {
			\WSAL\Helpers\Settings_Helper::set_option_value( 'reports-contact-name', $value );
		}
	}

	/**
	 * Retrieves the contact email for the reports.
	 *
	...
	public function get_contact_email() {
		return WSAL\Helpers\Settings_Helper::get_option_value( 'reports-contact-email', '' );
	}

	/**
	 * Updates the contact email for the reports.
	 *
	...
	public function set_contact_email( $value ) {
		if ( 0 === strlen( $value ) ) {
			\WSAL\Helpers\Settings_Helper::delete_option_value( 'reports-contact-email' );
		} else {
			\WSAL\Helpers\Settings_Helper::set_option_value( 'reports-contact-email', $value );
		}
	}

	/**
	 * Retrieves the contact phone number for the reports.
	 *
	...
	public function get_contact_phone_number() {
		return WSAL\Helpers\Settings_Helper::get_option_value( 'reports-contact-phone', '' );
	}

	/**
	 * Updates the contact phone number for the reports.
	 *
	 * @param string $value Contact phone number.
	 *
	 * @since 4.4.2.1
	 */
	public function set_contact_phone_number( $value ) {
		if ( 0 === strlen( $value ) ) {
			\WSAL\Helpers\Settings_Helper::delete_option_value( 'reports-contact-phone' );
		} else {
			\WSAL\Helpers\Settings_Helper::set_option_value( 'reports-contact-phone', $value );
		}
	}
	/* @premium:end */
