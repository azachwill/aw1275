...
final class WSAL_AlertManager {
	...
	public function __construct( WpSecurityAuditLog $plugin ) {
		...
		// phpcs:disable
		/* @premium:start */
		// phpcs:enable		// Hook for logging of scheduled events used by the Action Scheduler.
		// add_action( 'wsal/log_alert', array( $this, 'log_scheduled_event' ) );

		/*
		 * We need to earlier than Action Scheduler AJAX runner on shutdown hook to minimize delay in event processing.
		 * This will only apply to the delayed alerts.
		 *
		 * @see https://github.com/woocommerce/action-scheduler/blob/master/classes/ActionScheduler_QueueRunner.php#L70-L75
		 */
		/* @premium:end */
	 ...
	 * @return boolean
	 */
	public function has_triggered( $type, $count = 1 ) {
		return in_array( $type, $this->triggered_types ); // phpcs:ignore
	}
	...
	protected function log( $event_id, $event_data = array() ) {
		...
		$db_enabled_logger = false;
		// phpcs:ignore
		/* @premium:end */

		foreach ( $this->loggers as $logger ) {
			/* @premium:start */
			if ( ! ( $logger instanceof WSAL\Loggers\WSAL_Loggers_Database ) ) {
				$external_logger = true;
				if ( $logger instanceof WSAL_Ext_MirrorLogger ) {
					$logger->log( $event_id, $event_data );
				}
				// Skip non-database loggers.
				continue;
			}

			if ( $logger->is_external() ) {
				$external_logger = true;
				// Skip database logger if connected to external database.
				continue;
			}

			if ( ! self::$plugin->settings()->is_database_logging_enabled() ) {
				$db_enabled_logger = true;
		...
		/* @premium:start */
		// phpcs:enable
		// Add logging to other targets for later (mirrors, notifications etc.).
		if ( $external_logger || $db_enabled_logger ) {
			// Certain metadata needs to be truncated because Action Scheduler has a limit on the size of the task argument
			// of 8000 characters.
			$metadata_keys = array( 'MetaValue', 'MetaValueOld', 'MetaValueNew' );
			foreach ( $event_data as $meta_key => &$meta_value ) {
				if ( in_array( $meta_key, $metadata_keys, true ) && is_string( $meta_value ) ) {
					// Trim the meta value to the maximum length and append configured ellipses sequence.
					$meta_value = mb_strlen( $meta_value ) > 8000 ? ( mb_substr( $meta_value, 0, 7997 ) . '...' ) : $meta_value;
				}
			}
	...
	/* @premium:start */
	/**
	 * Logs to all loggers except for the internal and external database. These are process synchronously elsewhere.
	 *
	 * @param array $job_data An array of arguments passed through Action Scheduler. Contains "id" and "data".
	 * @since 4.3.0
	 *
	 * @deprecated 4.4.3 - Use \WSAL\Controllers\Alert_Manager::log_scheduled_event()
	 */
	public function log_scheduled_event( $job_data ) {
		_deprecated_function( __FUNCTION__, '4.4.3', '\WSAL\Controllers\Alert_Manager::log_scheduled_event()' );
		\WSAL\Controllers\Alert_Manager::log_scheduled_event( $job_data );
		// foreach ( $this->loggers as $logger ) {
		// 	if ( $logger instanceof WSAL\Loggers\WSAL_Loggers_Database ) {
		// 		if ( ! $logger->is_external() ) {
		// 			// Skip internal database logger.
		// 			continue;
		// 		}

		// 		if ( ! self::$plugin->settings()->is_database_logging_enabled() ) {
		// 			// Skip external database logger if logging is not enabled.
		// 			continue;
		// 		}
		// 	}

		// 	if ( $logger instanceof WSAL_Ext_MirrorLogger ) {
		// 		$logger->log_schedule( $job_data['id'], $job_data['data'] );
		// 	} else {
		// 		$logger->log( $job_data['id'], $job_data['data'] );
		// 	}
		// }
	}
	/* @premium:end */
