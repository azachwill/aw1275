...
class WSAL_Views_AuditLog extends WSAL_AbstractView {
	...
	public function admin_notices() {
				...
				<div class="updated wsal_notice">
					<div class="wsal_notice__wrapper">
						...
						<div class="wsal_notice__btns">
							...
							?>
							<a href="<?php echo esc_url( $trial_link ); ?>" class="button button-primary wsal_notice__btn notice-cta" target="_blank"><?php esc_html_e( 'Get the Free 14-day Premium trial', 'wp-security-audit-log' ); ?></a>
							<br>
							<a href="<?php echo esc_url( $buy_now ); ?>" class="start-trial-link" target="_blank"><?php esc_html_e( 'UPGRADE NOW', 'wp-security-audit-log' ); ?></a>
						</div>
						<!-- /.wsal_notice__btns -->
					</div>
		...
		// phpcs:enable
		if ( $is_current_view ) {
			WSAL_Connector_ConnectorFactory::display_notice_if_connection_not_available( $this->plugin );
			if ( class_exists( '\WSAL\Extensions\ExternalDB\Mirrors\WSAL_Ext_Mirrors_AWSCloudWatchConnection' ) ) {
				\WSAL\Extensions\ExternalDB\Mirrors\WSAL_Ext_Mirrors_AWSCloudWatchConnection::display_no_aws_sdk_notice( $this->plugin );
			}
			if ( class_exists( 'WSAL_Extension_Manager' ) ) {
				WSAL_Extension_Manager::display_helper_plugin_needed_notice( $this->plugin );
			}
		}
		// phpcs:disable
		/* @premium:end */
		...
		/* @premium:start */
		// phpcs:enable
		if ( in_array( $screen->base, array( 'toplevel_page_wsal-auditlog', 'wp-activity-log_page_wsal-usersessions-views', 'wp-activity-log_page_wsal-np-notifications', 'wp-activity-log_page_wsal-rep-views-main' ), true ) ) {
			$is_disabled = $this->plugin->settings()->is_database_logging_enabled();
			if ( ! $is_disabled ) {
				WSAL_Ext_Mirroring::display_no_database_being_used_notice();
			}
		}
		// phpcs:disable
		/* @premium:end */
	...
	public function AjaxInspector() {
		...
		if ( ! empty( $selected_db ) && 'archive' === $selected_db && get_current_user_id() === $selected_db_user ) {
			$this->plugin->settings()->switch_to_archive_db(); // Switch to archive DB.
		}
		// phpcs:disable
		/* @premium:end */
		// phpcs:enable
		...
		echo 'html, body { margin: 0; padding: 5px; color: #3c434a; font-family: -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen-Sans,Ubuntu,Cantarell,"Helvetica Neue",sans-serif; font-size: 14px; } body > div { padding: 15px; } body > div * { line-height: 1.6; } body > div r[g] { display: inline-block; } body > div t { display: none; } body > div r[g] r[r] { display: block; margin-left: 15px; }  body > div r[g] r[r] r { margin-right: 5px; }';
		echo '</style>';
		wsal_r( $alert_meta );
		echo '</body></html>';
		die;
	 ...
	 */
	public function ajax_switch_db() {
		// Filter $_POST array for security.
		$post_array = filter_input_array( INPUT_POST );

		// Verify nonce.
		if ( ! isset( $_REQUEST['nonce'] ) || false === wp_verify_nonce( \sanitize_text_field( \wp_unslash( $_REQUEST['nonce'] ) ), 'wsal-switch-db' ) ) {
		...
		if ( isset( $post_array['selected_db'] ) ) {
			WP_Helper::set_transient( 'wsal_wp_selected_db', $post_array['selected_db'], HOUR_IN_SECONDS );
			WP_Helper::set_transient( 'wsal_wp_selected_db_user', get_current_user_id(), HOUR_IN_SECONDS );
		}
	}
	// phpcs:disable
	/* @premium:end */
	...
	// phpcs:enable
	/**
	 * Method: Ajax request handler to dismiss missing AWS SDK nudge
	 *
	 * @since 4.3.2
	 */
	public function dismiss_missing_aws_sdk_nudge() {
		if ( ! $this->plugin->settings()->current_user_can( 'edit' ) ) {
			wp_send_json_error();
		}

		if ( ! array_key_exists( 'nonce', $_POST ) || ! wp_verify_nonce( $_POST['nonce'], 'dismiss_missing_aws_sdk_nudge' ) ) { // phpcs:ignore
			wp_send_json_error();
		}

		\WSAL\Helpers\Settings_Helper::delete_option_value( 'show-aws-sdk-config-nudge-4_3_2' );
		wp_send_json_success();
	}

	/**
	 * Method: Ajax request handler to dismiss helper plugin needed nudge
	 *
	 * @since 4.4.0
	 */
	public function dismiss_helper_plugin_needed_nudge() {
		if ( ! $this->plugin->settings()->current_user_can( 'edit' ) ) {
			wp_send_json_error();
		}

		if ( ! array_key_exists( 'nonce', $_POST ) || ! wp_verify_nonce( $_POST['nonce'], 'dismiss_helper_plugin_needed_nudge' ) ) { // phpcs:ignore
			wp_send_json_error();
		}
