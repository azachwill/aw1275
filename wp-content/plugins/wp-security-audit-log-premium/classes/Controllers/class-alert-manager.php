...
if ( ! class_exists( '\WSAL\Controllers\Alert_Manager' ) ) {
	...
	class Alert_Manager {
		...
		public static function init() {
			...
			/* @premium:start */
			// phpcs:enable
			// Hook for logging of scheduled events used by the Action Scheduler.
			add_action( 'wsal/log_alert', array( __CLASS__, 'log_scheduled_event' ) );
		...
		protected static function log( $event_id, $event_data = array() ) {
			...
			foreach ( self::get_loggers() as $logger ) {
				...
				if ( ! Settings_Helper::is_database_logging_enabled() ) {
		 ...
		 * @since 4.5.0
		 */
		public static function log_scheduled_event( $job_data ) {
			foreach ( self::get_loggers() as $logger ) {
				if ( $logger instanceof \WSAL\Loggers\WSAL_Loggers_Database ) {
					if ( ! $logger->is_external() ) {
