...
if ( ! class_exists( '\WSAL\Utils\Migration' ) ) {
	...
	class Migration extends Abstract_Migration {
		...
		protected static function migrate_up_to_4430() {
			...
			// Upgrade the confections from the older versions.
			if ( defined( 'WSAL_CONN_PREFIX' ) ) {
				$connections = Settings_Helper::get_options_by_prefix( \WSAL_CONN_PREFIX );
				foreach ( $connections as $connection ) {
					$connection->option_value = maybe_unserialize( $connection->option_value );
					if ( $connection->option_value instanceof \stdClass ) {
						$option_value = json_decode( json_encode( $connection->option_value ), true );
						Settings_Helper::set_option_value( $connection->option_name, $option_value );
					}
				}
			}
			// Upgrade the confections mirrors from the older versions.
			if ( defined( 'WSAL_CONN_PREFIX' ) ) {
				$mirrors = Settings_Helper::get_options_by_prefix( \WSAL_MIRROR_PREFIX );
				foreach ( $mirrors as $mirror ) {
					$mirror->option_value = maybe_unserialize( $mirror->option_value );
					if ( $mirror->option_value instanceof \stdClass ) {
						$option_value = json_decode( json_encode( $mirror->option_value ), true );
						Settings_Helper::set_option_value( $mirror->option_name, $option_value );
					}
				}
			}
			/* @premium:end */
