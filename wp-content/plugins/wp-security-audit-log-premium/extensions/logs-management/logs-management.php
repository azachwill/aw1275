	...
	public function delete_data_from_logs() {
		...
		if ( Settings_Helper::is_archiving_enabled() ) {
			if ( class_exists( 'WSAL_Ext_Common' ) ) {
				$wsal_archive_db = ( new WSAL_Ext_Common( $wsal, ( new WSAL_Ext_Plugin() ) ) )->get_archive_database_connection();
				$wsal->settings()->switch_to_archive_db();
				...
				if ( ! empty( $prepared_query ) ) {
					$deleted = $wsal_archive_db->query( $prepared_query );
					if ( ! empty( $deleted ) ) {
