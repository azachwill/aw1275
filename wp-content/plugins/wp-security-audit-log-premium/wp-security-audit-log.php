 ...
 *
 * @wordpress-plugin
 * Plugin Name: WP Activity Log (Premium)
 * Version:     4.5.3
...
if ( ! function_exists( 'wsal_freemius' ) ) {
	...
	if ( ! class_exists( 'WpSecurityAuditLog' ) ) {
		...
		class WpSecurityAuditLog {
			...
			/* @premium:start */
			/**
			 * Utility class for notifications extension.
			 *
			 * @var WSAL_NP_Common
			 * @since 4.2.1
			 */
			public $notifications_util;

			/**
			 * Utility class for external DB extension.
			 *
			 * @var WSAL_Ext_Common
			 * @since 4.3.0
			 */
			public $external_db_util;

			/**
			 * Utility class for the reports extension.
			 *
			 * @var WSAL_Rep_Common
			 * @since 4.3.1.1
			 */
			...
			/**
			 * Instance of the user sessions extension.
			 *
			 * @var WSAL_UserSessions_Plugin
			 * @since 4.3.4
			 */
			...
			public function init_hooks() {
				...
				/* @premium:start */
				add_filter( 'user_row_actions', array( $this, 'add_user_events_link' ), 10, 2 );
				/* @premium:end */
			}

			/* @premium:start */
			/**
			 * Adds a simple link to each user's row to view each users events.
			 *
			 ...
			 * @return string[]
			 */
			public function add_user_events_link( $actions, $user ) {
				if ( wsal_freemius()->has_active_valid_license() ) {
					$user_wsal_url = add_query_arg(
						array(
							'page'    => 'wsal-auditlog',
							'filters' => array( 'username:' . $user->user_login ),
						),
						admin_url( 'admin.php' )
					);

					$actions['add_user_events_link'] = '<a href="' . esc_url( $user_wsal_url ) . '">' . esc_attr__( 'See user\'s activity', 'wp-security-audit-log' ) . '</a>';
				}
					...
					if ( Settings_Helper::is_archiving_enabled() ) {
						...
						if ( ( ! isset( $get_page ) || 'wsal-auditlog' !== $get_page ) && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) ) {
							...
							if ( $selected_db && ( get_current_user_id() === $selected_db_user ) ) {
								...
								WP_Helper::delete_transient( 'wsal_wp_selected_db_user' );
							}
						}
					}
					/* @premium:end */
			...
			public function install() {
				...
				/* @premium:start */
				// if ( Settings_Helper::is_archiving_enabled() ) {
				// 	$this->settings()->switch_to_archive_db();
				// 	self::get_connector()->get_adapter( 'Occurrence' )->create_indexes();
				// 	self::get_connector()->get_adapter( 'Meta' )->create_indexes();
				// }
				/* @premium:end */
		...
		if ( file_exists( $prefixed_autoloader_file_path ) ) {
			...
			$action_scheduler_file_path = plugin_dir_path( __FILE__ ) . implode(
				DIRECTORY_SEPARATOR,
				array(
					'third-party',
					'woocommerce',
...
/* @premium:start */
// phpcs:enable
if ( ! function_exists( 'wsal_premium_on_plugin_activation' ) ) {
	/**
	 ...
	 * @since 4.4.3
	 */
	function wsal_premium_on_plugin_activation() {
		$premium_version_slug = 'wp-security-audit-log/wp-security-audit-log.php';
		if ( WP_Helper::is_plugin_active( $premium_version_slug ) ) {
			deactivate_plugins( $premium_version_slug, true );
		}
	}

	register_activation_hook( __FILE__, 'wsal_premium_on_plugin_activation' );
}
// phpcs:disable
/* @premium:end */
// phpcs:enable
