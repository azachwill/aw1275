...
class CloudWatch extends AbstractProcessingHandler
{
     ...
     * @throws \Exception
     */
    public function __construct(CloudWatchLogsClient $client, $group, $stream, $retention = 14, $batchSize = 10000, array $tags = [], $level = Logger::DEBUG, $bubble = \true, $createGroup = \true)
    {
        if ($batchSize > 10000) {
    ...
    private function formatRecords(array $entry) : array
    {
        ...
        foreach ($entries as $entry) {
            $records[] = ['message' => $entry, 'timestamp' => $timestamp];
        }
        return $records;
    }
    /**
    ...
    private function send(array $entries) : void
    {
        ...
        $data = ['logGroupName' => $this->group, 'logStreamName' => $this->stream, 'logEvents' => $entries];
    ...
    private function initializeGroup() : void
    {
        ...
        if (!\in_array($this->group, $existingGroupsNames, \true)) {
            ...
            if ($this->retention !== null) {
                $this->client->putRetentionPolicy(['logGroupName' => $this->group, 'retentionInDays' => $this->retention]);
            }
        }
    }
    ...
    private function refreshSequenceToken() : void
    {
        ...
        $existingStreams = $this->client->describeLogStreams(['logGroupName' => $this->group, 'logStreamNamePrefix' => $this->stream])->get('logStreams');
        ...
        if (!\in_array($this->stream, $existingStreamsNames, \true)) {
            $this->client->createLogStream(['logGroupName' => $this->group, 'logStreamName' => $this->stream]);
        }
    ...
    {
        return new LineFormatter("%channel%: %level_name%: %message% %context% %extra%", null, \false, \true);
    }
    /**
     * {@inheritdoc}
     */
