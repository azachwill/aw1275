...
use WSAL_Vendor\GuzzleHttp\Exception\ConnectException;
use WSAL_Vendor\GuzzleHttp\Exception\RequestException;
use WSAL_Vendor\GuzzleHttp\Promise as P;
use WSAL_Vendor\GuzzleHttp\Promise\FulfilledPromise;
use WSAL_Vendor\GuzzleHttp\Promise\PromiseInterface;
use WSAL_Vendor\GuzzleHttp\Psr7\LazyOpenStream;
use WSAL_Vendor\GuzzleHttp\TransferStats;
use WSAL_Vendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \WSAL_Vendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \WSAL_Vendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
