...
use WSAL_Vendor\JmesPath\Env;
use WSAL_Vendor\JmesPath\DebugRuntime;
$description = <<<EOT
...
if (isset($args['file']) || isset($args['suite']) || isset($args['case'])) {
    ...
    $json = \json_decode(\file_get_contents($path), \true);
    $set = $json[$args['suite']];
    $data = $set['given'];
    if (!isset($expression)) {
    ...
    $data = \json_decode(\stream_get_contents(\STDIN), \true);
} else {
    die($description);
}
